...
namespace LearnArray.Tests
{
    ...
    public class AlistTest<T> where T : IList, new()
    {
        ...
        public void TestPrivateSumMethod()
        {
            // 1. Получаем Type класса, содержащего приватный метод
            Type type = typeof(LList2);

            // 2. Получаем MethodInfo для приватного метода
            // Имя метода: "Sum"
            // Флаги: NonPublic (приватный), Instance (не статический)
            MethodInfo methodInfo = type.GetMethod("Sum", BindingFlags.NonPublic | BindingFlags.Instance);

            // Проверяем, что метод найден
            if (methodInfo == null)
            {
                Assert.Fail("Приватный метод 'Sum' не найден.");
                return;
            }
            ...
            // 3. Вызываем приватный метод
            // Первый аргумент: экземпляр объекта, на котором вызывается метод (list)
            // Второй аргумент: массив параметров
            object result = methodInfo.Invoke(list, parameters);
            ...
            // Дополнительный тест
            object[] parameters2 = new object[] { -10, 2 };
            int expectedResult2 = -8;
            object result2 = methodInfo.Invoke(list, parameters2);
            Assert.AreEqual(expectedResult2, (int)result2);
        }
